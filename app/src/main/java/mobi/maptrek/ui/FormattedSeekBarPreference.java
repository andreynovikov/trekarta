 ...
 */
public class FormattedSeekBarPreference extends Preference {
    ...
    public FormattedSeekBarPreference(
            @NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr,
            int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.FormattedSeekBarPreference, defStyleAttr, defStyleRes);
        // The ordering of these two statements are important. If we want to set max first, we need
        // to perform the same steps by changing min/max to max/min as following:
        // mMax = a.getInt(...) and setMin(...).
        mMin = a.getInt(R.styleable.FormattedSeekBarPreference_min, 0);
        setMax(a.getInt(R.styleable.FormattedSeekBarPreference_android_max, 100));
        setSeekBarIncrement(a.getInt(R.styleable.FormattedSeekBarPreference_seekBarIncrement, 0));
        mAdjustable = a.getBoolean(R.styleable.FormattedSeekBarPreference_adjustable, true);
        mShowSeekBarValue = a.getBoolean(R.styleable.FormattedSeekBarPreference_showSeekBarValue, false);
        mUpdatesContinuously = a.getBoolean(R.styleable.FormattedSeekBarPreference_updatesContinuously, false);
        mFormat = a.getString(R.styleable.FormattedSeekBarPreference_format);
        if (mFormat == null)
            mFormat = "%d";
        a.recycle();
    }
    ...
    /**
     * Gets whether the {@link FormattedSeekBarPreference} should continuously save the {@link SeekBar} value
     ...
     *
     * @return Whether the {@link FormattedSeekBarPreference} should continuously save the {@link SeekBar}
    ...
    /**
     * Sets whether the {@link FormattedSeekBarPreference} should continuously save the {@link SeekBar} value
     * while it is being dragged.
     *
     * @param updatesContinuously Whether the {@link FormattedSeekBarPreference} should continuously save
    ...
    void updateLabelValue(int value) {
        if (mSeekBarValueTextView != null) {
            mSeekBarValueTextView.setText(String.format(mFormat, value));
        }
    }
